...
def Forces(Axes, EField, DipoleSep, DipPol):
    ...
    Force=EField
    for i in range(25,26): #Iterate through all x values
        for j in range(22,23): #Iterate through all y values
            for k in range(28,29): #Iterate through all z values
      ...
      #      for k in range(len(EField[2])): #Iterate through all z values
                if (not np.isnan(Axes[i,j,k,0])): #Checks to see if the value is a dipole
                    ...
                    if ((CentralDifference[0]==True)&(CentralDifference[1]==True)):
                        ...
                        print(Force[i,j,k,0])
                        print(DipPol[i,j,k,0]*(np.conjugate((EField[(i+1),j,k,0]-EField[(i-1),j,k,0])/(2*DipoleSep))))
                        Force[i,j,k,0] += DipPol[i,j,k,1]*(np.conjugate((EField[(i+1),j,k,1]-EField[(i-1),j,k,1])/(2*DipoleSep)))
                        print(Force[i,j,k,0])
                        print(DipPol[i,j,k,1]*(np.conjugate((EField[(i+1),j,k,1]-EField[(i-1),j,k,1])/(2*DipoleSep))))
                        Force[i,j,k,0] += DipPol[i,j,k,2]*(np.conjugate((EField[(i+1),j,k,2]-EField[(i-1),j,k,2])/(2*DipoleSep)))
                        print(Force[i,j,k,0])
                        print(DipPol[i,j,k,2]*(np.conjugate((EField[(i+1),j,k,2]-EField[(i-1),j,k,2])/(2*DipoleSep))))
                    ...
                    print(Force[i,j,k,0])
                    print(CentralDifference[0])
                    print(CentralDifference[1])
                    ...
                    if ((CentralDifference[0]==True)&(CentralDifference[1]==True)):
                        ...
                        print(Force[i,j,k,1])
                        print(DipPol[i,j,k,0]*(np.conjugate((EField[i,(j+1),k,0]-EField[i,(j-1),k,0])/(2*DipoleSep))))
                        Force[i,j,k,1] += DipPol[i,j,k,1]*(np.conjugate((EField[i,(j+1),k,1]-EField[i,(j-1),k,1])/(2*DipoleSep)))
                        print(Force[i,j,k,1])
                        print(DipPol[i,j,k,1]*(np.conjugate((EField[i,(j+1),k,1]-EField[i,(j-1),k,1])/(2*DipoleSep))))
                        Force[i,j,k,1] += DipPol[i,j,k,2]*(np.conjugate((EField[i,(j+1),k,2]-EField[i,(j-1),k,2])/(2*DipoleSep)))
                        print(Force[i,j,k,1])
                        print(DipPol[i,j,k,2]*(np.conjugate((EField[i,(j+1),k,2]-EField[i,(j-1),k,2])/(2*DipoleSep))))
                    ...
                    if ((CentralDifference[0]==True)&(CentralDifference[1]==True)):
                        ...
                        print(Force[i,j,k,2])
                        print(DipPol[i,j,k,0]*(np.conjugate((EField[i,j,(k+1),0]-EField[i,j,(k-1),0])/(2*DipoleSep))))
                        Force[i,j,k,2] += DipPol[i,j,k,1]*(np.conjugate((EField[i,j,(k+1),1]-EField[i,j,(k-1),1])/(2*DipoleSep)))
                        print(Force[i,j,k,2])
                        print(DipPol[i,j,k,1]*(np.conjugate((EField[i,j,(k+1),1]-EField[i,j,(k-1),1])/(2*DipoleSep))))
                        Force[i,j,k,2] += DipPol[i,j,k,2]*(np.conjugate((EField[i,j,(k+1),2]-EField[i,j,(k-1),2])/(2*DipoleSep)))
                        print(Force[i,j,k,2])
                        print(DipPol[i,j,k,2]*(np.conjugate((EField[i,j,(k+1),2]-EField[i,j,(k-1),2])/(2*DipoleSep))))
...
while (dpl<Final_dpl):
    ...
    callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size 2 -dpl "+str(dpl)+" -sym enf -lambda 1 -prop 0 0 1 -store_beam -store_dip_pol -store_int_field" #The script for performing the DDA calculations
    print(".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size 2 -dpl "+str(dpl)+" -sym enf -lambda 1 -prop 0 0 1 -store_beam -store_dip_pol -store_int_field")
 ...
 #   DipFiles, IntFFiles, BeamFiles, ForceFiles = sorted(glob.glob(DipPathInput))[-1], sorted(glob.glob(IntFPathInput))[-1], sorted(glob.glob(BeamPathInput))[-1], sorted(glob.glob(ForcePathInput))[-1] #File containing the paths to each DipPol, IntField file
    ...
    print(len(EFieldGridValues))
    CalcForce = Forces(AxesGrid,EFieldGridValues,DipSeperation,DipPolGridValues)
    EndTime=time.clock()
    '''print(RadForceGridValues[33,33,33,0])
    print(RadForceGridValues[33,33,33,1])
    print(RadForceGridValues[33,33,33,2])'''
    print(CalcForce[25,22,28,0])
    print(CalcForce[25,22,28,1])
    print(CalcForce[25,22,28,2])
    
    #SAVE CALCULATED FORCES
    #ESTIMATED PARTICLE FORCE
    
    #This section is where we look at the ADDA Calculated Forces
    dpl=dpl+Step_dpl #DELETE ME
