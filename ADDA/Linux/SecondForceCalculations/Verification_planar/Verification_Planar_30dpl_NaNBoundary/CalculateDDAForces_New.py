...
def Grid(PositionsArray,GridType):
    ...
    for i in range(3):
        ...
        GridSize[i] = np.round((maxLocation-minLocation)/Sep)+3
    if (GridType=='float'):
...
while (dpl<Final_dpl):
    ...
    for i in range(len(DipPolRawPositions[1])):
        xLocation=np.int(np.round((DipPolRawPositions[0,i]-min_xLocation)/DipSeperation))+1 #Find the x Location in the grid
        yLocation=np.int(np.round((DipPolRawPositions[1,i]-min_yLocation)/DipSeperation))+1 #Find the y Location in the grid
        zLocation=np.int(np.round((DipPolRawPositions[2,i]-min_zLocation)/DipSeperation))+1 #Find the z Location in the grid
    ...
    for i in range(len(IntFieldRawPositions[1])):
        xLocation=np.int(np.round((IntFieldRawPositions[0,i]-min_xLocation)/DipSeperation))+1 #Find the x Location in the grid
        yLocation=np.int(np.round((IntFieldRawPositions[1,i]-min_yLocation)/DipSeperation))+1 #Find the y Location in the grid
        zLocation=np.int(np.round((IntFieldRawPositions[2,i]-min_zLocation)/DipSeperation))+1 #Find the z Location in the grid
    ...
    for i in range(len(BeamRawPositions[1])):
        xLocation=np.int(np.round((BeamRawPositions[0,i]-min_xLocation)/DipSeperation))+1 #Find the x Location in the grid
        yLocation=np.int(np.round((BeamRawPositions[1,i]-min_yLocation)/DipSeperation))+1 #Find the y Location in the grid
        zLocation=np.int(np.round((BeamRawPositions[2,i]-min_zLocation)/DipSeperation))+1 #Find the z Location in the grid
    ...
    for i in range(len(RadForceRawPositions[1])):
        xLocation=np.int(np.round((RadForceRawPositions[0,i]-min_xLocation)/DipSeperation))+1 #Find the x Location in the grid
        yLocation=np.int(np.round((RadForceRawPositions[1,i]-min_yLocation)/DipSeperation))+1 #Find the y Location in the grid
        zLocation=np.int(np.round((RadForceRawPositions[2,i]-min_zLocation)/DipSeperation))+1 #Find the z Location in the grid
