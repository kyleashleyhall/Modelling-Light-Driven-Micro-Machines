            ...
            for k in range(len(EField[2])): #Iterate through all z values
                if (not np.isnan(Axes[i,j,k,0])): #Checks to see if the value is a dipole
                    ...
                    if ((CentralDifference[0]==True)&(CentralDifference[1]==True)):
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[(i+1),j,k,0]-EField[(i-1),j,k,0])/(2*DipoleSep)))
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[(i+1),j,k,1]-EField[(i-1),j,k,1])/(2*DipoleSep)))
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[(i+1),j,k,2]-EField[(i-1),j,k,2])/(2*DipoleSep)))
                        Force[i,j,k,0] = 0.5*np.real(Force[i,j,k,0])
                    elif ((CentralDifference[0]==False)&(CentralDifference[1]==True)):
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[(i+1),j,k,0]-EField[i,j,k,0])/(DipoleSep)))
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[(i+1),j,k,1]-EField[i,j,k,1])/(DipoleSep)))
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[(i+1),j,k,2]-EField[i,j,k,2])/(DipoleSep)))
                        Force[i,j,k,0] = 0.5*np.real(Force[i,j,k,0])
                    elif ((CentralDifference[0]==True)&(CentralDifference[1]==False)):
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,j,k,0]-EField[(i-1),j,k,0])/(DipoleSep)))
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,j,k,1]-EField[(i-1),j,k,1])/(DipoleSep)))
                        Force[i,j,k,0] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,j,k,2]-EField[(i-1),j,k,2])/(DipoleSep)))
                        Force[i,j,k,0] = 0.5*np.real(Force[i,j,k,0])
                    else:
                        Force[i,j,k,0] = 0
                    ...
                    if ((CentralDifference[0]==True)&(CentralDifference[1]==True)):
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,(j+1),k,0]-EField[i,(j-1),k,0])/(2*DipoleSep)))
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,(j+1),k,1]-EField[i,(j-1),k,1])/(2*DipoleSep)))
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,(j+1),k,2]-EField[i,(j-1),k,2])/(2*DipoleSep)))
                        Force[i,j,k,1] = 0.5*np.real(Force[i,j,k,1])
                    elif ((CentralDifference[0]==False)&(CentralDifference[1]==True)):
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,(j+1),k,0]-EField[i,j,k,0])/(DipoleSep)))
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,(j+1),k,1]-EField[i,j,k,1])/(DipoleSep)))
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,(j+1),k,2]-EField[i,j,k,2])/(DipoleSep)))
                        Force[i,j,k,1] = 0.5*np.real(Force[i,j,k,1])
                    elif ((CentralDifference[0]==True)&(CentralDifference[1]==False)):
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,j,k,0]-EField[i,(j-1),k,0])/(DipoleSep)))
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,j,k,1]-EField[i,(j-1),k,1])/(DipoleSep)))
                        Force[i,j,k,1] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,j,k,2]-EField[i,(j-1),k,2])/(DipoleSep)))
                        Force[i,j,k,1] = 0.5*np.real(Force[i,j,k,1])
                    else:
                        Force[i,j,k,1] = 0
                    ...
                    if ((CentralDifference[0]==True)&(CentralDifference[1]==True)):
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,j,(k+1),0]-EField[i,j,(k-1),0])/(2*DipoleSep)))
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,j,(k+1),1]-EField[i,j,(k-1),1])/(2*DipoleSep)))
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,j,(k+1),2]-EField[i,j,(k-1),2])/(2*DipoleSep)))
                        Force[i,j,k,2] = 0.5*np.real(Force[i,j,k,2])
                    elif ((CentralDifference[0]==False)&(CentralDifference[1]==True)):
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,j,(k+1),0]-EField[i,j,k,0])/(DipoleSep)))
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,j,(k+1),1]-EField[i,j,k,1])/(DipoleSep)))
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,j,(k+1),2]-EField[i,j,k,2])/(DipoleSep)))
                        Force[i,j,k,2] = 0.5*np.real(Force[i,j,k,2])
                    elif ((CentralDifference[0]==True)&(CentralDifference[1]==False)):
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,0])*(np.conjugate((EField[i,j,k,0]-EField[i,j,(k-1),0])/(DipoleSep)))
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,1])*(np.conjugate((EField[i,j,k,1]-EField[i,j,(k-1),1])/(DipoleSep)))
                        Force[i,j,k,2] += DipPol*(EField[i,j,k,2])*(np.conjugate((EField[i,j,k,2]-EField[i,j,(k-1),2])/(DipoleSep)))
                        Force[i,j,k,2] = 0.5*np.real(Force[i,j,k,2])
                    else:
                        Force[i,j,k,2] = 0
...
while (dpl<Final_dpl):
    ...
    DipFiles, IntFFiles, BeamFiles, ForceFiles, LogFile = sorted(glob.glob(DipPathInput))[-1], sorted(glob.glob(IntFPathInput))[-1], sorted(glob.glob(BeamPathInput))[-1], sorted(glob.glob(ForcePathInput))[-1], sorted(glob.glob(LogPathInput))[-1] #File containing the paths to each DipPol, IntField file
    ...
    CalcForce = Forces(AxesGrid,EFieldGridValues,DipSeperation,DipolePolarizability)
    EndTime=time.clock()
