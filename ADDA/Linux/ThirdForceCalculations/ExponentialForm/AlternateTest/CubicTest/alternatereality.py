...
CalculatedForce = Forces(IntField, IncBeam, DipPol, DipSep(IntField[:,0]))        
EndTime_OurCalc=time.clock()

#SAVE CALCULATED FORCES
with open('CalculatedForce','wb') as f:
...
ADDADipoleForceFile = np.loadtxt('RadForce-Y', skiprows=1)
...
ForceError=np.zeros([1,7])
TimeRecordings=np.zeros([1,2])
ForceError[0,0] = 30 
ForceError[0,1] = pow((pow(ADDAParticleForce[0]-EstimatedParticleForce[0],2)),0.5)
ForceError[0,2] = pow((pow(ADDAParticleForce[1]-EstimatedParticleForce[1],2)),0.5)
ForceError[0,3] = pow((pow(ADDAParticleForce[2]-EstimatedParticleForce[2],2)),0.5)
ForceError[0,4] = pow((pow(ADDAParticleForce[0]-EstimatedParticleForce[0],2)),0.5)/ADDAParticleForce[0]
ForceError[0,5] = pow((pow(ADDAParticleForce[1]-EstimatedParticleForce[1],2)),0.5)/ADDAParticleForce[1]
ForceError[0,6] = pow((pow(ADDAParticleForce[2]-EstimatedParticleForce[2],2)),0.5)/ADDAParticleForce[2]
TimeRecordings[0,0] = 30
TimeRecordings[0,1] = EndTime_OurCalc-StartTime_OurCalc

with open('ForceError', 'wb') as f:
    ...
    np.savetxt(f, ForceError, fmt='%lf', delimiter=' ')
with open('TimeLog', 'wb') as f:
    f.write(b'dpl OurCalcTime\n')
    np.savetxt(f, TimeRecordings, fmt='%lf', delimiter=' ')
