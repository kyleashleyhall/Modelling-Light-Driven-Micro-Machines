...
R=Initial_R #Set the value of y to the initial value
...
dpl=Initial_dpl
TimeRecordings=np.zeros([(((Final_R-Initial_R)//Step_dpl)+1),6])
while (dpl<Final_dpl):
    while (R<Final_R):
        ...
        if(theta==0):
            ...
            Force_R=np.sqrt(np.square(Force_x)+np.square(Force_y))
        else:
            Force_R=0-np.sqrt(np.square(Force_x)+np.square(Force_y))
        ...
        EstimatedParticleForce=np.array([[x],[y],[z],[R],[Force_x],[Force_y],[Force_z],[Force_R]])
        try:
