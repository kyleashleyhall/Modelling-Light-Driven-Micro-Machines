...
while(BeamWidth<0.601):
    ...
    while (Arbvalue == 0):
        ...
        TimeRecordings = np.append(TimeRecordings, np.array([[iteration,z,Force_z,EndTime_ADDA-StartTime_ADDA,EndTime_OurCalc-StartTime_OurCalc]]), axis=0)
        ...
        with open(TimeLogPath, 'wb') as f:
            f.write(b'Iteration# z Force_z ADDATime OurCalcTime\n')
            np.savetxt(f, TimeRecordings, fmt='%e', delimiter=' ')
