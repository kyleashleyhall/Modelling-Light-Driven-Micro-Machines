...
while (Arbvalue == 0):
    ...
    Tracker = np.savetxt('Tracker', 'Keeps track of what its doing\n')
    ...
    if Force_z > 0:
        z += Step_z
        Tracker = open('Tracker', 'w')
        print('Moved beam up', file=Tracker)
        Tracker.close()
    else:
        Step_z *= 0.5
        z -= Step_z
        Tracker = open('Tracker', 'w')
        print('Moved beam down, halved increment', file=Tracker)
        Tracker.close()
