...
zValues=np.zeros(0)
GridSize_z=0
while (z<(Final_z+Step_z)):
    GridSize_z+=1
    zValues=np.append(zValues,z)
    z+=Step_z
    
xForce_Grid=np.zeros([GridSize_z,GridSize_x])
yForce_Grid=np.zeros([GridSize_z,GridSize_x])
zForce_Grid=np.zeros([GridSize_z,GridSize_x])
...
zPositionsPath = str(os.getcwd())+str(os.sep+'zPositions')
np.savetxt(zPositionsPath,zValues, fmt='%e',delimiter=' ')