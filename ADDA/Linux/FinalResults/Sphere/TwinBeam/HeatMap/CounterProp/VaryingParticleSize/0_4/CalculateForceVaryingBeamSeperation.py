...
ParticleSize=((float(os.getcwd()[-3]))/1)+((float(os.getcwd()[-1]))/10)
...
callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 1 -m 1.183390347 0 -store_beam -store_dip_pol -store_int_field -store_force" #The script for performing the DDA calculations
...
while (BeamSeperation<(Final_BeamSeperation+Step_BeamSeperation)):
    ...
    while (y<(Final_y+Step_y)):
        ...
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 1 -m 1.183390347 0 -beam barton5 "+str(BeamWidth)+" "+str(x_beam1)+" "+str(y_beam1)+" "+str(z_beam1)+" -store_beam" #The script for performing the DDA calculations
        subprocess.call(callString,shell=True)
        ...
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 1 -m 1.183390347 0 -beam barton5 "+str(BeamWidth)+" "+str(x_beam2)+" "+str(y_beam2)+" "+str(z_beam2)+" -store_beam" #The script for performing the DDA calculations
        subprocess.call(callString,shell=True)
        ...
        #Calculate incident beam due to third beam
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 -1 -m 1.183390347 0 -beam barton5 "+str(BeamWidth)+" "+str(x_beam1)+" "+str(y_beam1)+" "+str(z_beam1)+" -store_beam" #The script for performing the DDA calculations
        subprocess.call(callString,shell=True)
        Beam3File = sorted(glob.glob(BeamPathInput))[-1]
        Beam3Raw=np.transpose(np.loadtxt(Beam3File, skiprows=1))
        try:
            shutil.rmtree(Beam3File.replace(os.sep+'IncBeam-Y',''))
        except:
            print('Cannot Delete')
        
        #Calculate incident beam due to fourth beam
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 -1 -m 1.183390347 0 -beam barton5 "+str(BeamWidth)+" "+str(x_beam2)+" "+str(y_beam2)+" "+str(z_beam2)+" -store_beam" #The script for performing the DDA calculations
        subprocess.call(callString,shell=True)
        Beam4File = sorted(glob.glob(BeamPathInput))[-1]
        Beam4Raw=np.transpose(np.loadtxt(Beam4File, skiprows=1))
        try:
            shutil.rmtree(Beam4File.replace(os.sep+'IncBeam-Y',''))
        except:
            print('Cannot Delete')
        
        #Calculate the new beam file
        IncBeamRawTemp2=np.zeros([np.size(Beam1Raw,axis=0),np.size(Beam1Raw,axis=1)])
        IncBeamRawTemp1=np.zeros([np.size(Beam1Raw,axis=0),np.size(Beam1Raw,axis=1)])
        ...
        for iterator in range(4,10): #Bring across the total E_inc beam
            IncBeamRawTemp1[iterator,:]=np.add(Beam1Raw[iterator,:],Beam2Raw[iterator,:])
            IncBeamRawTemp2[iterator,:]=np.add(IncBeamRawTemp1[iterator,:],Beam3Raw[iterator,:])
            IncBeamRaw[iterator,:]=np.add(IncBeamRawTemp2[iterator,:],Beam4Raw[iterator,:])
        ...
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 1 -m 1.183390347 0 -sym enf -beam read DualBeam -store_dip_pol -store_int_field" #The script for performing the DDA calculations
        print(".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 1 -m 1.183390347 0 -sym enf -beam read DualBeam -store_dip_pol -store_int_field")
        subprocess.call(callString,shell=True)
