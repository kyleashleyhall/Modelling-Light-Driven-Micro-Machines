...
Force = np.transpose(np.loadtxt('TotalForcePowerHalf'))
...
while t_0 <= t_end:
    pos_index = np.argmin(np.abs(y_position - Force[0]))
    Force_y = Force[1,pos_index]
    if pos_index == 0:
        Brownian = np.abs(BrownianForce(Drag_Coefficient, Temperature, sigma))
    elif pos_index == len(Force[:,1])-1:
        Brownian = -np.abs(BrownianForce(Drag_Coefficient, Temperature, sigma))
    else:   
...
np.savetxt('PositionsPowerHalf', PositionArray, fmt='%e', delimiter='  ')