...
dpl=Initial_dpl
TimeRecordings=np.zeros([(((Final_x-Initial_x)//Step_dpl)+1),5])
while (dpl<Final_dpl):
    while (x<Final_x):
        print('Processing dpl: '+str(dpl))
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size 2 -shape read propellorfile -lambda 1.064 -m 1.183390347 0 -prop 0 0 1 -beam barton5 "+str(BeamWidth)+" "+str(-x)+" "+str(y)+" "+str(z)+" -store_beam -store_dip_pol -store_int_field" #The script for performing the DDA calculations
        print(".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size 2 -shape read propellorfile -lambda 1.064 -m 1.183390347 0 -prop 0 0 1 -beam barton5 "+str(BeamWidth)+" "+str(-x)+" "+str(y)+" "+str(z)+" -store_beam -store_dip_pol -store_int_field")
        ...
        TimeRecordings[(((x-Initial_x)//Step_dpl)),0] = x
        TimeRecordings[(((x-Initial_x)//Step_dpl)),1] = y
        TimeRecordings[(((x-Initial_x)//Step_dpl)),2] = z
        TimeRecordings[(((x-Initial_x)//Step_dpl)),3] = EndTime_ADDA-StartTime_ADDA
        TimeRecordings[(((x-Initial_x)//Step_dpl)),4] = EndTime_OurCalc-StartTime_OurCalc
