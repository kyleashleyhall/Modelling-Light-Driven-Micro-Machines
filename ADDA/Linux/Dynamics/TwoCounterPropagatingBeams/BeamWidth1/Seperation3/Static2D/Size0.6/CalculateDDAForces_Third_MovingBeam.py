...
ParticleSize=float(os.getcwd()[-3])+((float(os.getcwd()[-1]))/10)
...
callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -prop 0 0 1 -m 1.18339034696 0 -store_beam -store_dip_pol -store_int_field -store_force" #The script for performing the DDA calculations
...
CorrectionFactor=((((ADDAParticleForce[0])**2)+((ADDAParticleForce[1])**2)+((ADDAParticleForce[2])**2))**0.5)/((((EstimatedParticleForce[0])**2)+((EstimatedParticleForce[1])**2)+((EstimatedParticleForce[2])**2))**0.5)
...
x_beam2, y_beam2, z_beam2 = 0,-3,0

#Particle start point start point
x_position, y_position, z_position = x_0,y_0,0
...
while x_0 <= x_end:
    ...
    while y_0 <= y_end:
        ...
        print('Processing y position: '+str(y_0))
        
        #Calculate incident beam due to first beam
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -m 1.18339034696 0 -lambda 1.064 -prop 0 0 1 -beam barton5 "+str(BeamWidth)+" "+str(x_beam1)+" "+str(y_beam1)+" "+str(z_beam1)+" -store_beam" #The script for performing the DDA calculations
        subprocess.call(callString,shell=True)
        Beam1AFile = sorted(glob.glob(BeamPathInput))[-1]
        Beam1ARaw=np.transpose(np.loadtxt(Beam1AFile, skiprows=1))
        try:
            shutil.rmtree(Beam1AFile.replace(os.sep+'IncBeam-Y',''))
        except:
            print('Cannot Delete')
        ...
        try:
            shutil.rmtree(Beam1BFile.replace(os.sep+'IncBeam-Y',''))
        except:
            print('Cannot Delete')
        
        #First beam total
        IncBeam1Raw=np.zeros([np.size(Beam1ARaw,axis=0),np.size(Beam1ARaw,axis=1)])
        for i in range(0,3): #Bring across the x,y,z positions
            IncBeam1Raw[i,:]=Beam1ARaw[i,:]
        ...
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -m 1.18339034696 0 -lambda 1.064 -prop 0 0 1 -beam barton5 "+str(BeamWidth)+" "+str(x_beam2)+" "+str(y_beam2)+" "+str(z_beam2)+" -store_beam" #The script for performing the DDA calculations
        subprocess.call(callString,shell=True)
        Beam2AFile = sorted(glob.glob(BeamPathInput))[-1]
        Beam2ARaw=np.transpose(np.loadtxt(Beam2AFile, skiprows=1))
        try:
            shutil.rmtree(Beam2AFile.replace(os.sep+'IncBeam-Y',''))
        except:
            print('Cannot Delete')
        ...
        try:
            shutil.rmtree(Beam2BFile.replace(os.sep+'IncBeam-Y',''))
        except:
            print('Cannot Delete')
        
        #Second beam total
        IncBeam2Raw=np.zeros([np.size(Beam2ARaw,axis=0),np.size(Beam2ARaw,axis=1)])
        for i in range(0,3): #Bring across the x,y,z positions
            IncBeam2Raw[i,:]=Beam2ARaw[i,:]
        for i in range(4,10): #Bring across the total E_inc beam
            IncBeam2Raw[i,:]=np.add(Beam2ARaw[i,:],Beam2BRaw[i,:])
        
        #Calculate the new beam file
        IncBeamRaw=np.zeros([np.size(IncBeam2Raw,axis=0),np.size(IncBeam2Raw,axis=1)])
        for i in range(0,3): #Bring across the x,y,z positions
            IncBeamRaw[i,:]=IncBeam2Raw[i,:]
        for i in range(4,10): #Bring across the total E_inc beam
            IncBeamRaw[i,:]=np.add(IncBeam1Raw[i,:],IncBeam2Raw[i,:])
        ...
        callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -m 1.18339034696 0 -lambda 1.064 -prop 0 0 1 -sym enf -beam read DualBeam -store_dip_pol -store_int_field" #The script for performing the DDA calculations
        print(".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -m 1.18339034696 0 -lambda 1.064 -prop 0 0 1 -sym enf -beam read DualBeam -store_dip_pol -store_int_field")
        subprocess.call(callString,shell=True)
        ...
        PotentialArray = np.append(PotentialArray, np.array([[x_0,y_0,EstimatedParticleForce3[0],EstimatedParticleForce3[1],EstimatedParticleForce3[2]]]),axis=0)
