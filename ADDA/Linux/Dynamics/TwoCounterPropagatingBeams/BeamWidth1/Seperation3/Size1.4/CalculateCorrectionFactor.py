...
particleDiameterValue=float((os.getcwd()[-3:])) #Varies the Electric Field strength
...
for iterator in range(iterations):
    ...
    CorrectionFactors[iterator,0] = 30
...
CorrectionFactorsPath = str(os.getcwd())+str(os.sep+'CorrectionFactorsSize'+str(particleDiameterValue)) #|(F_ADDA - F_Calc)|/F_ADDA	
with open(CorrectionFactorsPath, 'wb') as f:
    f.write(b'gridsize Lambda(micro m) RefractiveIndex ParticleDiameter(micro m) CorrectionFactor\n')
