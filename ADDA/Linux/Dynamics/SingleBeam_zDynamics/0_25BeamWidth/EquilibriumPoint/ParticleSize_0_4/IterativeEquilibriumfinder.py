...
#             Force[i,5]=0.5*np.real(((DipPol[4,i]+(1j*DipPol[5,i]))*dEx_dy)+((DipPol[6,i]+(1j*DipPol[7,i]))*dEy_dy)+((DipPol[8,i]+(1j*DipPol[9,i]))*dEz_dy))
#==============================================================================
        """Forces in x and y have been commented out as they are irrelevant to the equilibrium position for a cone"""
        Force[i,4] = 0
        Force[i,5] = 0
...
while (Arbvalue == 0):
    ...
    callString=".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -m 1.18339034696 0 -prop 0 0 1 -beam barton5 "+str(BeamWidth)+" "+str(x)+" "+str(y)+" "+str(-z)+" -store_beam -store_dip_pol -store_int_field" #The script for performing the DDA calculations
    print(".."+os.sep+"src"+os.sep+"seq"+os.sep+"adda -size "+str(ParticleSize)+" -grid 30 -lambda 1.064 -m 1.18339034696 0 -prop 0 0 1 -beam barton5 "+str(BeamWidth)+" "+str(x)+" "+str(y)+" "+str(-z)+" -store_beam -store_dip_pol -store_int_field")
    ...
    if(iteration==1):
        MinForce=np.sqrt(np.square(Force_z))
