...
def BrownianForce(Dragcoefficient, tempertature):
    ...
    return np.sqrt(2*Dragcoefficient*(Boltzmann)*(tempertature+273))*random.gauss(0,1e-4)
...
def eqnmotion(Torque, t_step, dragcoef, r):
    return (t_step*Torque)/(dragcoef*r)
...
#==============================================================================
    #Torques
    Torques = CalculateTorque(CalculatedForce[:,0], CalculatedForce[:,1], CalculatedForce[:,2], CalculatedForce[:,4], CalculatedForce[:,5], CalculatedForce[:,6])                
    Torque_z = np.sum(Torques[:,5])
    Dtheta = eqnmotion(Torque_z, t_step, Drag_Coefficient, r)
    
    #Calculate the change in angle of the particle
    theta += -Dtheta
    t_0 += t_step
...
np.savetxt('AngularOrientations', AngleArray, fmt='%e', delimiter=' ')
EndTime=time.clock()
TimeRecordings=np.zeros([1,1])
TimeRecordings[0,0]=EndTime-StartTime
TimeLogPath = str(os.getcwd())+str(os.sep+'TimeLog')	
with open(TimeLogPath, 'wb') as f:
    f.write(b'Time Taken (s)\n')
