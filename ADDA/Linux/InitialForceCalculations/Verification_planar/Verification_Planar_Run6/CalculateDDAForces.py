	  ...
	  #Calculating the force
		for j in range(3): #Force in each direction	
			...
			for k in range(3): #Summating the different contributions
				y = (0.5*(np.real((DipPol[k,i])*(np.conjugate(dE[k,j,i]))))) - c
				t = Force[j,i] + y
				c = (t - Force[j,i]) - y 
				Force[j,i] = t 
				#Force[j,i]=Force[j,i]+np.round((0.5*(np.real((DipPol[k,i])*(np.conjugate(dE[k,j,i]))))),decimals=5)
...
while (dpl<Final_dpl):
	...
	ParticleForce = np.array([[sum(CalcForce[0])],[sum(CalcForce[1])],[sum(CalcForce[2])]])
	"""try:
