!=======================================================================
! Module mpi_subs.f v2
! This module contains four main subroutines 
!    SUBROUTINE COLSUM
!    SUBROUTINE SHARE0
!    SUBROUTINE SHARE1
!    SUBROUTINE SHARE2
...
! 13.12.28 (BTD) following Sergy Ruzankin:
!                * added NRFLD to arg list of SHARE1
!                * added CALL MPI_BCAST_INT(NRFLD,1,0,IERR)
! 14.01.06 (BTD) following Mike Wolff
!                * added NORICHAR to arg list of SHARE1
! end history
!
! Copyright (C) 2002,2003,2004,2007,2008,2013,2014 B.T. Draine and M. Collinge
!
...
!    SMORI_1(1-4,1-4,1-NSCAT)= sum of Mueller matrix for orientations done
!                              by MYID 
...
!    SMORI(1-4,1-4,1-NSCAT)  = running sum of Mueller matrix for orientations
!                              done by MYID
...
!    SMORI(1-4,1-4,1-NSCAT)  = updated Mueller matrix sum over orientations
!
...
!=============================================================================
    ...
    SUBROUTINE SHARE1(A1,A2,A3,AEFFA,AK1,BETA,BETADF,BETMID,BETMXD,CALPHA,     &
                      ...
                      MXNY,MXNZ,MXPBC,MXPHI,MXRAD,MXSCA,MXTHET,MXWAV,MYID,     &
                      NAMBIENT,NAT,NAT0,NAT3,NBETA,NBETH,NCOMP,NORI,NORICHAR,  &
                      NPHI,NRAD,NRFLD,NSCAT,NSMELTS,NTHETA,NWAV,NX,NY,NZ,      &
...
! dimensioning information:
      ...
      INTEGER :: IANISO,IDVERR,IDVOUT,ILIN10,ILIN12,INIT,IOBIN,IORTH,   &
                 IORI0,IPBC,IRAD0,IWAV0,IWRKSC,IWRPOL,JPBC,MXITER,MYID, &
                 NAT,NAT0,NAT3,NBETA,NBETH,NCOMP,NORI,NORICHAR,         &
                 NPHI,NRAD,NRFLD,NSCAT,NSMELTS,NTHETA,NWAV,NX,NY,NZ
      ...
      REAL(WP) :: AK1,BETMID,BETMXD,DAEFF,ETASCA,GAMMA,NAMBIENT,PHIMID,PHIMXD, &
...
! 13.12.28 (BTD) mpi_subs_v2.f90
!                following Sergey Ruzankin
!                * added NRFLD to argument list of SHARE1
!                * added CALL MPI_BCAST_INT(NRFLD,1,0,IERR)
! 15.04.06 (BTD) * added AK1 to argument list of SHARE1
!                * added CALL MPI_BCAST_REAL(AK1,1,0,IERR)

! end history
! Copyright (C) 2002,2003,2004,2005,2007,2008,2012,2013,2015
...
!      write(0,*)'share1 ckpt 1 CDESCR=',CDESCR
!      write(0,*)'share1 ckpt 1 CFLEPS(1)=',CFLEPS(1)
!***
...
!      write(0,*)'share1 ckpt 4, CDESCR=',CDESCR
!***
      CALL MPI_BCAST_CHAR(CFLEPS,60*MXCOMP,0,IERR)
!*** diagnostic
      write(0,*)'share1 ckpt 5, myid=',myid
      write(0,*)'share1 ckpt 5, CFLEPS(1)=',CFLEPS(1)
!***
...
!***
      ...
      CALL MPI_BCAST_INT(NORICHAR,1,0,IERR)
      CALL MPI_BCAST_INT(NPHI,1,0,IERR)
      CALL MPI_BCAST_INT(NRAD,1,0,IERR)
      CALL MPI_BCAST_INT(NRFLD,1,0,IERR)
!*** diagnostic
...
!***
      CALL MPI_BCAST_REAL(SINGLE,AK1,1,0,IERR)
