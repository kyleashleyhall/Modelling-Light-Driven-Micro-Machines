...
!                * modified to print JPBC and NCOMP near top of
!                  wxxxrxxxkxxx.sca file to facilitate handling by
!                  program checker
! 14.10.22 (BTD) * modified to write out JRTQ = 0 or 1 depending on
!                  whether torques have been calculated or not
! 14.11.25 (BTD) * modified output formatting to eliminate blank space
!                  at beginning of lines beginning with JO=1, JO=2, or
!                  mean
!                * change Qtrqab -> Qtrqinc
!                * change Qtrqsc -> Qtrqsca
! 15.02.05 (BTD) * modified some output formats to align 'JO=2' and 'mean'
! end history
...
!*** diagnostic
!      write(0,*)'writesca_v9 ckpt 1, myid=',myid
...
!*** diagnostic
!         write(0,*)'writesca_v9 ckpt 2, myid=',myid
!***
         IF(IWRKSC==1)THEN
...
!*** diagnostic
!            write(0,*)'writesca_v9 ckpt 2.1, myid=',myid,' cflsca=',cflsca
!            write(0,*)'                   itheta=',itheta
!            write(0,*)'                   iphi=',iphi
!***
            OPEN(UNIT=8,FILE=CFLSCA,STATUS='UNKNOWN')

            WRITE(8,FMT=9020)CSTAMP,CDESCR,CALPHA,CMDSOL,CSHAPE,JPBC,JTRQ, &
                             NCOMP,NAT0,DAEFF,DPHYS
...
! 13.06.25 (BTD) need to calculate total scattering cross section per
!                unit length
               WRITE(8,9350)
            ELSEIF(JPBC==3)THEN
         ...
         IF(CMDTRQ=='DOTORQ')THEN
            IF(IWRKSC==1)THEN
               WRITE(8,FMT=9250)(QTRQAB(J,1),J=1,3),(QTRQSC(J,1),J=1,3), &
                                (QTRQAB(J,1)+QTRQSC(J,1),J=1,3)
               IF(IORTH==2)WRITE(8,FMT=9255)(QTRQAB(J,2),J=1,3),    &
                           (QTRQSC(J,2),J=1,3),                     &
                           (QTRQAB(J,2)+QTRQSC(J,2),J=1,3),         &
...
!*** diagnostic
!                  write(0,*)'writesca_v9 ckpt 3, nd=',nd
...
!*** diagnostic
!                  write(0,*)'writesca_v9 ckpt 4, sm(nd,1,1)=',sm(nd,1,1)
!***
         ...
         IF(IORTH==1)THEN
            IF(JPBC==0)THEN
               ...
               IF(CMDTRQ=='DOTORQ')WRITE(8,FMT=9250)(QTRQABSUM(J,1),J=1,3), &
                                   (QTRQSCSUM(J,1),J=1,3),                  &
                                   (QTRQABSUM(J,1)+QTRQSCSUM(J,1),J=1,3)
            ELSE
               WRITE(8,FMT=9151)
            ENDIF
            WRITE(8,FMT=9060)
...
!*** diagnostic
!            write(0,*)'writesca_v9 ckpt 5, about to open qtable...'
!***
            ...
            ENDIF
!*** diagnostic
!            write(0,*)'writesca_v9 ckpt 6, about to write to qtable...'
!***
            IF(JPBC==0)THEN
...
9020  FORMAT(                                                    &
         ...
         ' ',A,' --- shape',/,                                   &
         I4,' = JPBC',/,                                         &
         I4,' = JTRQ (= 1 if torques are calculated)',/,         &
         I4,' = NCOMP = number of compositions',/,               &
         I8,5X,'= NAT0 = number of dipoles',/,                   &
...
9040  FORMAT(                                                &
         'BETA =',F8.3,' = rotation of target around A1',/, &
         'THETA=',F8.3,' = angle between A1 and k',/,       &
         ' PHI =',F8.3,' = rotation of A1 around k')
9041  FORMAT(                                                            &
...
9055  FORMAT(                                                  &
         'JO=2:',1P,3E12.4,E12.4,2E11.4,E12.4,/,'mean:',       &
         4E12.4,2E11.4,E12.4,/,'Qpol=',E12.4,52X,'dQpha=',E12.4)
9056  FORMAT(1P,E11.4,4E11.4,E12.4,E11.4,E11.4,I6)
9057  FORMAT(1P,E11.4,E11.4,3E12.4)
9060  FORMAT(                                                            &
...
9150  FORMAT(8X,'Qsca*g(1)   Qsca*g(2)   Qsca*g(3)   iter  mxiter',2X, &
        'Nsca',/,'JO=1:',1P,3E12.4,3I7)
9151  FORMAT(42X,' iter  mxiter')
9155  FORMAT('JO=2:',1P,3E12.4,3I7,/,'mean:',1P,3E12.4)
9250  FORMAT(7X,'Qtrqinc(1) Qtrqinc(2) Qtrqinc(3) ', &
        'Qtrqsca(1) Qtrqsca(2) Qtrqsca(3) ',         &
     &  '  Qtrq(1)    Qtrq(2)    Qtrq(3)',/,         &
     &  'JO=1:',1P,9E11.3)
9255  FORMAT('JO=2:',1P,9E11.3,/,'mean:',1P,9E11.3)
