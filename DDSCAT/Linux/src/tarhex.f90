...
! 13.05.30 (BTD): Small change in formatting of WRITE to CMSGN
!                 I4 -> I7 to avoid overflow
! 13.06.17 (BTD): Another small change in formatting of WRITE to CMSGN
!                 to avoid overflow
! end history

! Copyright (C) 1993,1995,1996,1997,1998,2000,2006,2007,2008,2013
...
! subroutine WRIMSG
      ...
      CALL WRIMSG('TARHEX',CMSGNM)
      WRITE(CMSGNM,FMT='(A,I7,A,I7,A,I7,A,F7.4)')' NAT=',NAT,' NFAC=', &
                                NFAC,' NLAY=',NLAY,' aspect ratio=',ASPR
      CALL WRIMSG('TARHEX',CMSGNM)
      WRITE(CDESCR,FMT='(A,I7,A,I7,A,I7,A,F7.4)')' Hex prism,NAT=',NAT, &
...
! Now store atom coordinates in file
      ...
      ENDIF
      RETURN
9020  FORMAT(' Hexagonal Prism:  A=',F10.4,' B=',F10.4,/,            &
         I10,' = NAT ',/,                                          &
         3F10.6,' = A_1 vector',/,                                 &
         3F10.6,' = A_2 vector',/,                                 &
         3F10.6,' = lattice spacings (d_x,d_y,d_z)/d',/,           &
         3F11.5,' = lattice offset x0(1-3) = (x_TF,y_TF,z_TF)/d ', &
               'for dipole 0 0 0',/,                               &
