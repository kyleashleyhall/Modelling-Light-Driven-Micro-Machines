...
!      WRITE(IDVOUT,*)'  GAUSS_SPH = gaussian sphere target'
      ...
      WRITE(IDVOUT,*)'  LAYRDSLAB = multilayer rectangular slab'
      WRITE(IDVOUT,FMT='(A,A)')'  MLTBLOCKS = construct from cubic blocks', &
                     ' input from file "blocks.par"'
      WRITE(IDVOUT,FMT='(A)')'  OCTHEDRON = regular octahedron'
      WRITE(IDVOUT,FMT='(A,A)')'  ONIONSHEL = onion-like shell with', &
                     ' central sphere either vacuum or other material'
      ...
      WRITE(IDVOUT,FMT='(A,A)')'  UNIAXICYL = ',          &
                     'finite cylinder of uniaxial material'
      WRITE(IDVOUT,FMT='(A,A)')'  UNIAXIHEX = ',                  &
                     'finite hexagonal prism of unixaxial material'
      ...
      IF(CSHAPE=='ANIELLIPS'.OR.CSHAPE=='aniellips')THEN
         ...
         DO J=1,100
            ...
            CALL TARGET(A1,A2,CSHAPE,CFLSHP,IDVSHP,IOSHP,CDESCR, &
                        ...
                        NAT3,PYD,PZD,BETADF,PHIDF,THETADF,X0,    &
                        IANISO,NCOMP_NEED)
...
!-----------------------------------------------------------------------
      ELSEIF(CSHAPE=='LAYRDSLAB'.OR.CSHAPE=='layrdslab')THEN
         ...
         DO J=1,100
            ...
            WRITE(IDVOUT,*)'      (for one layer slab set f1=1; f2=f3=f4=0)'
            WRITE(IDVOUT,*)'      (for bilayer slab set f1,f2; f3=f4=0)'
            WRITE(IDVOUT,*)'      (for trilayer slab set f1,f2,f3; f4=0)'
...
!-----------------------------------------------------------------------
      ELSEIF(CSHAPE=='MLTBLOCKS'.OR.CSHAPE=='mltblocks')THEN
         ...
         STOP
!-----------------------------------------------------------------------
      ELSEIF(CSHAPE=='OCTHEDRON'.OR.CSHAPE=='octhedron')THEN
         CSHAPE='OCTHEDRON'
         WRITE(IDVOUT,'(A,A)')'OCTHEDRON : regular octahedron',        &
             ' each face is an equilateral triangle with side length = s'
         WRITE(IDVOUT,*)'   enter s/d' 
         READ(*,*)SHPAR(1)
         CALL TARGET(A1,A2,CSHAPE,CFLSHP,IDVSHP,IOSHP,CDESCR, &
                     MXNAT,SHPAR,DX,NAT,IXYZ,ICOMP,IDVOUT,    &
                     NAT3,PYD,PZD,BETADF,PHIDF,THETADF,X0,    &
         ...
         WRITE(IDVOUT,7000)NAT
!-----------------------------------------------------------------------
      ELSEIF(CSHAPE=='ONIONSHEL'.OR.CSHAPE=='onionshel')THEN
         CSHAPE='ONIONSHEL'
         WRITE(IDVOUT,'(A,A)')'ONIONSHEL : spherical shell of uniaxial ',      &
            'material, with central sphere either vacuum or other core material'
         WRITE(IDVOUT,*)'   enter ICORE=0 or 1 for central void or core'
         READ(*,*)SHPAR(1)
         WRITE(IDVOUT,*)'   enter (outer radius)/d'
         READ(*,*)SHPAR(2)
         WRITE(IDVOUT,*)'   enter (inner radius)/(outer radius)'
         READ(*,*)SHPAR(3)
         CALL TARGET(A1,A2,CSHAPE,CFLSHP,IDVSHP,IOSHP,CDESCR, &
                     MXNAT,SHPAR,DX,NAT,IXYZ,ICOMP,IDVOUT,    &
                     NAT3,PYD,PZD,BETADF,PHIDF,THETADF,X0,    &
